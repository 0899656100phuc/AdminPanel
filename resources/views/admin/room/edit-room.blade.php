                        ...
                        <div class="col-lg-3">
                            ...
                            <div class="mb-3">
                                    ...
                                    @foreach ($rooms->images as $item)
                                        ...
                                        <img src="{{ '/images/room/' . $item->image }}" width="70" height="70"
                                            alt="Image" srcset="">
                                    @endforeach
                            ...
                            <div class="col-lg-9">
                                <form action="{{ url('update-room/' . $rooms->id) }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="hotel" class="form-control" id="">
                                            @foreach ($hotels as $item)
                                                <option value="{{$item->id}}" {{ $rooms->hotel_id == $item->id ? 'selected' : '' }}>{{$item->id}} / {{$item->name}}</option>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="typeroom" class="form-control" id="">
                                            @foreach ($typeRoom as $item)
                                                <option value="{{$item->id}}"  {{ $rooms->type_room_id  == $item->id ? 'selected' : '' }}>{{$item->id}} / {{$item->name}}</option>
                                            @endforeach
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="status">
                                            <option value="0" @if($rooms->status == 0) selected @endif>Còn phòng trống</option>
                                            <option value="1" @if($rooms->status == 1) selected @endif>Đã được đặt</option>
